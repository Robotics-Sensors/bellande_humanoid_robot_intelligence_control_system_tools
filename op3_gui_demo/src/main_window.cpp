...
MainWindow::MainWindow(int argc, char **argv, QWidget *parent)
    : QMainWindow(parent), qnode_op3_(argc, argv), is_updating_(false),
      is_walking_(false) {
  // code to DEBUG
  debug_ = false;
  ...
  QObject::connect(
      &qnode_op3_,
      SIGNAL(updateWalkingParameters(op3_walking_module_msgs::WalkingParam)),
      this, SLOT(updateWalkingParams(op3_walking_module_msgs::WalkingParam)));

  /*********************
   ** Logging
   **********************/
  ui_.view_logging->setModel(qnode_op3_.loggingModel());
  QObject::connect(&qnode_op3_, SIGNAL(loggingUpdated()), this,
...
void MainWindow::setHeadAngle() {
  if (is_updating_ == true)
    return;
  qnode_op3_.setHeadJoint(ui_.head_pan_slider->value() * M_PI / 180,
...
void MainWindow::on_actionAbout_triggered() {
  QMessageBox::about(this, tr("About ..."),
                     tr("<h2>OP3 Demo 0.10</h2><p>Copyright ROBOTIS</p>"));
}
...
void MainWindow::initModeUnit() {
  ...
  for (std::map<int, std::string>::iterator module_it =
           qnode_op3_.module_table_.begin();
  ...
  QObject::connect(get_mode_button, SIGNAL(clicked(bool)), &qnode_op3_,
...
void MainWindow::initMotionUnit() {
  ...
  for (std::map<int, std::string>::iterator motion_it =
           qnode_op3_.motion_table_.begin();
    ...
    std::map<int, int>::iterator shortcut_it =
        qnode_op3_.motion_shortcut_table_.find(motion_index);
  ...
  QObject::connect(signal_mapper, SIGNAL(mapped(int)), &qnode_op3_,
                   SLOT(playMotion(int)));
