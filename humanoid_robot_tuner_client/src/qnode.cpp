...
#include "../include/humanoid_robot_tuner_client/qnode.hpp"
...
QNode::QNode(int argc, char **argv)
    : init_argc_(argc), init_argv_(argv), is_refresh_(false) {}
...
bool QNode::init() {
  ros::init(init_argc_, init_argv_, "humanoid_robot_tuner_client");
...
void QNode::sendTorqueEnableMsg(
    humanoid_robot_tuning_module_msgs::JointTorqueOnOffArray msg) {
...
void QNode::sendJointGainDataMsg(humanoid_robot_tuning_module_msgs::JointOffsetData msg) {
...
void QNode::getPresentJointOffsetData(bool recalculate_offset) {
  ...
  humanoid_robot_tuning_module_msgs::GetPresentJointOffsetData
      _get_present_joint_offset_data;

  // request

  // response
  if (get_present_joint_offset_data_client_.call(
          _get_present_joint_offset_data)) {
    for (int id = 0;
         id < _get_present_joint_offset_data.response.present_data_array.size();
         id++) {
      humanoid_robot_tuning_module_msgs::JointOffsetPositionData _temp =
...
void QNode::parseOffsetGroup(const std::string &path) {
  ...
  for (YAML::iterator yaml_it = right_arm_node.begin();
       yaml_it != right_arm_node.end(); ++yaml_it) {
    int index;
  ...
  for (YAML::iterator yaml_it = left_arm_node.begin();
       yaml_it != left_arm_node.end(); ++yaml_it) {
    int index;
    std::string joint_name;
  ...
  for (YAML::iterator yaml_it = legs_node.begin(); yaml_it != legs_node.end();
       ++yaml_it) {
    int index;
    std::string joint_name;
  ...
  for (YAML::iterator yaml_it = body_node.begin(); yaml_it != body_node.end();
       ++yaml_it) {
    int index;
    std::string joint_name;
