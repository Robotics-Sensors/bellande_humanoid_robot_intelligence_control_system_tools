...
void QNodeHUMANOID_ROBOT::init_default_demo(ros::NodeHandle &ros_node) {
  ...
  std::string motion_path =
      ros_node.param<std::string>("gui_motion", default_motion_path);
  parseMotionMapFromYaml(motion_path);

  ROS_INFO("Initialized node handle for default demo");
}

void QNodeHUMANOID_ROBOT::updateHeadJointStatesCallback(
...
void QNodeHUMANOID_ROBOT::setHeadJoint(double pan, double tilt) {
...
// Walking
void QNodeHUMANOID_ROBOT::setWalkingCommand(const std::string &command) {
...
void QNodeHUMANOID_ROBOT::refreshWalkingParam() {
...
void QNodeHUMANOID_ROBOT::saveWalkingParam() {
...
void QNodeHUMANOID_ROBOT::applyWalkingParam(
...
// Motion
void QNodeHUMANOID_ROBOT::playMotion(int motion_index) {
...
// Demo
void QNodeHUMANOID_ROBOT::setDemoCommand(const std::string &command) {
...
void QNodeHUMANOID_ROBOT::parseMotionMapFromYaml(const std::string &path) {
  YAML::Node doc;
  try {
    // load yaml
  ...
  for (YAML::iterator yaml_it = motion_sub_node.begin();
       yaml_it != motion_sub_node.end(); ++yaml_it) {
    int motion_index;
  ...
  for (YAML::iterator _it = _shoutcut_sub_node.begin();
       _it != _shoutcut_sub_node.end(); ++_it) {
