...
#include "../include/humanoid_robot_gui_demo/preview_walking_form.h"

PreviewWalkingForm::PreviewWalkingForm(QWidget *parent)
    : QWidget(parent), p_walking_ui(new Ui::PreviewWalkingForm),
      is_updating_(false) {
...
void PreviewWalkingForm::on_button_set_walking_param_clicked(bool check) {
  ...
  qnode_humanoid_robot_->sendWalkingParamMsg(msg);
}
...
void PreviewWalkingForm::on_button_send_body_offset_clicked(bool check) {
  ...
  qnode_humanoid_robot_->sendBodyOffsetMsg(msg);
}
...
void PreviewWalkingForm::on_button_send_foot_distance_clicked(bool check) {
  ...
  qnode_humanoid_robot_->sendFootDistanceMsg(msg);
}

void PreviewWalkingForm::on_button_p_walking_init_pose_clicked(bool check) {
  std::string ini_pose_path =
      ros::package::getPath(ROS_PACKAGE_NAME) + "/config/init_pose.yaml";
  qnode_humanoid_robot_->parseIniPoseData(ini_pose_path);

  std_msgs::Bool msg;
  msg.data = true;

  qnode_humanoid_robot_->sendResetBodyMsg(msg);
}
...
void PreviewWalkingForm::on_button_p_walking_balance_on_clicked(bool check) {
  ...
  qnode_humanoid_robot_->sendWholebodyBalanceMsg(msg);
}
...
void PreviewWalkingForm::on_button_footstep_plan_clicked(bool check) {
  ...
  qnode_humanoid_robot_->makeFootstepUsingPlanner(target_pose);
}

void PreviewWalkingForm::on_button_footstep_clear_clicked(bool check) {
  qnode_humanoid_robot_->clearFootsteps();
}
...
void PreviewWalkingForm::on_button_footstep_go_clicked(bool check) {
  ...
  qnode_humanoid_robot_->setWalkingFootsteps(step_time);
...
void PreviewWalkingForm::sendPWalkingCommand(const std::string &command,
                                             bool set_start_foot) {
  ...
  msg.step_angle =
      p_walking_ui->dSpinBox_p_walking_step_angle->value() * DEG2RAD;
  msg.start_leg =
      set_start_foot ? p_walking_ui->comboBox_p_walking_start_leg->currentText()
                           .toStdString()
                     : "";
...
void PreviewWalkingForm::makeInteractiveMarker() {
  ...
  qnode_humanoid_robot_->makeInteractiveMarker(current_pose);
}
...
void PreviewWalkingForm::updateInteractiveMarker() {
  ...
  qnode_humanoid_robot_->updateInteractiveMarker(current_pose);
}
...
void PreviewWalkingForm::clearMarkerPanel() {
  ...
  qnode_humanoid_robot_->clearInteractiveMarker();
}
...
void PreviewWalkingForm::setPoseToMarkerPanel(
  ...
  Eigen::Vector3d euler =
      rad2deg<Eigen::Vector3d>(quaternion2rpy(current.orientation));
...
void PreviewWalkingForm::getPointFromMarkerPanel(
...
void PreviewWalkingForm::setPointToMarkerPanel(
...
Eigen::Vector3d
PreviewWalkingForm::rotation2rpy(const Eigen::MatrixXd &rotation) {
  Eigen::Vector3d rpy;

  rpy[0] = atan2(rotation.coeff(2, 1), rotation.coeff(2, 2));
  rpy[1] = atan2(-rotation.coeff(2, 0), sqrt(pow(rotation.coeff(2, 1), 2) +
...
Eigen::MatrixXd PreviewWalkingForm::rpy2rotation(const double &roll,
                                                 const double &pitch,
                                                 const double &yaw) {
  Eigen::MatrixXd rotation =
      rotationZ(yaw) * rotationY(pitch) * rotationX(roll);

  return rotation;
}

Eigen::Quaterniond
PreviewWalkingForm::rpy2quaternion(const Eigen::Vector3d &euler) {
  return rpy2quaternion(euler[0], euler[1], euler[2]);
}

Eigen::Quaterniond PreviewWalkingForm::rpy2quaternion(const double &roll,
                                                      const double &pitch,
                                                      const double &yaw) {
...
Eigen::Quaterniond
PreviewWalkingForm::rotation2quaternion(const Eigen::MatrixXd &rotation) {
  Eigen::Matrix3d rotation3d;
...
Eigen::Vector3d
PreviewWalkingForm::quaternion2rpy(const Eigen::Quaterniond &quaternion) {
  Eigen::Vector3d rpy = rotation2rpy(quaternion.toRotationMatrix());

  return rpy;
}

Eigen::Vector3d PreviewWalkingForm::quaternion2rpy(
...
Eigen::MatrixXd
PreviewWalkingForm::quaternion2rotation(const Eigen::Quaterniond &quaternion) {
