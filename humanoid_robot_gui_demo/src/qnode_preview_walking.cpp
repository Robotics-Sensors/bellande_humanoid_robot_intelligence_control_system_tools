...
void QNodeHUMANOID_ROBOT::init_preview_walking(ros::NodeHandle &ros_node) {
  ...
  rviz_clicked_point_sub_ = ros_node.subscribe(
      "clicked_point", 0, &QNodeHUMANOID_ROBOT::pointStampedCallback, this);
  interactive_marker_server_.reset(
      new interactive_markers::InteractiveMarkerServer("Feet_Pose", "", false));
...
bool QNodeHUMANOID_ROBOT::transformPose(const std::string &from_id,
  ...
  try {
    ...
    Eigen::Quaterniond transform_orientation(
        desired_transform.getRotation().w(),
    ...
    if (inverse == false) {
      after_position = transform_orientation.inverse().toRotationMatrix() *
                       (before_position - transform_position);
      after_orientation = before_orientation * transform_orientation.inverse();
    } else {
      after_position =
          transform_orientation.toRotationMatrix() * before_position +
          transform_position;
...
// demo
void QNodeHUMANOID_ROBOT::pointStampedCallback(
...
// interactive marker
void QNodeHUMANOID_ROBOT::interactiveMarkerFeedback(
  ...
  case visualization_msgs::InteractiveMarkerFeedback::POSE_UPDATE: {
    ...
    bool result =
        transformPose("/world", "/body_link", feedback->pose, local_pose);
    if (result == false) {
      log(Warn, "transformation is failed.");
...
void QNodeHUMANOID_ROBOT::makeInteractiveMarker(const geometry_msgs::Pose &marker_pose) {
  ...
  bool result =
      transformPose("/world", "/body_link", marker_pose, world_pose, true);
  if (result == false)
  ...
  center_marker_control.interaction_mode =
  ...
  interactive_control.interaction_mode =
  ...
  interactive_marker_server_->setCallback(
      ...
      boost::bind(&QNodeHUMANOID_ROBOT::interactiveMarkerFeedback, this, _1));
...
bool QNodeHUMANOID_ROBOT::updateInteractiveMarker(const geometry_msgs::Pose &pose) {
  ...
  result_getting =
      interactive_marker_server_->get(marker_name_, interactive_marker);
...
void QNodeHUMANOID_ROBOT::getInteractiveMarkerPose() {
  ...
  // transform : world to local
  geometry_msgs::Pose local_pose;
  bool result = transformPose("/world", "/body_link", _interactive_marker.pose,
                              local_pose);
  if (result == false)
...
void QNodeHUMANOID_ROBOT::clearInteractiveMarker() {
...
// footstep
void QNodeHUMANOID_ROBOT::setWalkingFootsteps(const double &step_time) {
  ...
  humanoid_robot_online_walking_module_msgs::Step2DArray footsteps;

  for (int ix = 0; ix < preview_foot_steps_.size(); ix++) {
    humanoid_robot_online_walking_module_msgs::Step2D step;
...
void QNodeHUMANOID_ROBOT::clearFootsteps() {
...
void QNodeHUMANOID_ROBOT::makeFootstepUsingPlanner() {
  makeFootstepUsingPlanner(current_pose_);
}

void QNodeHUMANOID_ROBOT::makeFootstepUsingPlanner(
    const geometry_msgs::Pose &target_foot_pose) {
  // foot step service
  ...
  if (humanoid_footstep_client_.call(get_step)) {
    if (get_step.response.result) {
      for (int ix = 0; ix < get_step.response.footsteps.size(); ix++) {
        ...
        // log footsteps
        msg_stream << "Foot Step #" << ix + 1 << " [ " << foot_string << "] - ["
                   << foot_pose.x << ", " << foot_pose.y << " | "
...
void QNodeHUMANOID_ROBOT::visualizePreviewFootsteps(bool clear) {
  ...
  rviz_marker.action = (clear == false) ? visualization_msgs::Marker::ADD
  ...
  for (int ix = preview_foot_types_.size() - 1; ix >= 0; ix--) {
    ...
    if (!clear) {
      Eigen::Vector3d marker_position(preview_foot_steps_[ix].x,
                                      preview_foot_steps_[ix].y, height);
      Eigen::Vector3d marker_position_offset;

      Eigen::Vector3d toward(1, 0, 0),
          direction(cos(preview_foot_steps_[ix].theta),
                    sin(preview_foot_steps_[ix].theta), 0);
      Eigen::Quaterniond marker_orientation(
          Eigen::Quaterniond::FromTwoVectors(toward, direction));
          ...
          humanoid_robot_online_walking_module_msgs::Step2D::LEFT_FOOT_SWING) // left
      {
        ...
        marker_position_offset =
            marker_orientation.toRotationMatrix() * offset_y;
      } else if (preview_foot_types_[ix] ==
                 humanoid_robot_online_walking_module_msgs::Step2D::
                     RIGHT_FOOT_SWING) // right
      {
      ...
      tf::quaternionEigenToMsg(marker_orientation,
                               rviz_marker.pose.orientation);
...
// Preview walking
void QNodeHUMANOID_ROBOT::sendFootStepCommandMsg(
...
void QNodeHUMANOID_ROBOT::sendWalkingParamMsg(
    humanoid_robot_online_walking_module_msgs::WalkingParam msg) {
  walking_param_pub_.publish(msg);
  log(Info, "Set Walking Parameter");
}

void QNodeHUMANOID_ROBOT::sendBodyOffsetMsg(geometry_msgs::Pose msg) {
  body_offset_pub_.publish(msg);
  log(Info, "Send Body Offset");
}

void QNodeHUMANOID_ROBOT::sendFootDistanceMsg(std_msgs::Float64 msg) {
  foot_distance_pub_.publish(msg);
  log(Info, "Send Foot Distance");
}

void QNodeHUMANOID_ROBOT::sendResetBodyMsg(std_msgs::Bool msg) {
  reset_body_msg_pub_.publish(msg);
  log(Info, "Reset Body Pose");
}

void QNodeHUMANOID_ROBOT::sendWholebodyBalanceMsg(std_msgs::String msg) {
  wholebody_balance_pub_.publish(msg);
  log(Info, "Wholebody Balance Msg");
}

void QNodeHUMANOID_ROBOT::parseIniPoseData(const std::string &path) {
  YAML::Node doc;
  try {
    // load yaml
  ...
  for (YAML::iterator it = tar_pose_node.begin(); it != tar_pose_node.end();
       ++it) {
