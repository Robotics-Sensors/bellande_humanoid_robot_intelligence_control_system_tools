...
QNodeHUMANOID_ROBOT::QNodeHUMANOID_ROBOT(int argc, char **argv)
    : init_argc_(argc), init_argv_(argv), body_height_(-1.0) {
  // code to DEBUG
  debug_ = false;
...
QNodeHUMANOID_ROBOT::~QNodeHUMANOID_ROBOT() {
  if (ros::isStarted()) {
...
bool QNodeHUMANOID_ROBOT::init() {
  ros::init(init_argc_, init_argv_, "humanoid_robot_gui_demo");
  ...
  std::string default_config_path =
      ros::package::getPath(ROS_PACKAGE_NAME) + "/config/gui_config.yaml";
  std::string config_path =
      ros_node.param<std::string>("gui_config", default_config_path);
  parseJointNameFromYaml(config_path);

  // start time
  start_time_ = ros::Time::now();

  // Come Back Right Here Later
...
void QNodeHUMANOID_ROBOT::parseJointNameFromYaml(const std::string &path) {
  YAML::Node doc;
  try {
    // load yaml
  ...
  for (YAML::iterator _it = id_sub_node.begin(); _it != id_sub_node.end();
       ++_it) {
    int joint_id;
  ...
  std::vector<std::string> modules =
      doc["module_list"].as<std::vector<std::string>>();

  int module_index = 0;
  for (std::vector<std::string>::iterator modules_it = modules.begin();
...
std::string QNodeHUMANOID_ROBOT::getModeName(const int &index) {
  std::string mode = "";
...
int QNodeHUMANOID_ROBOT::getModeIndex(const std::string &mode_name) {
  int mode_index = -1;
  std::map<std::string, int>::iterator map_it =
      mode_index_table_.find(mode_name);
...
int QNodeHUMANOID_ROBOT::getModeSize() { return index_mode_table_.size(); }

// number of joints
int QNodeHUMANOID_ROBOT::getJointSize() { return id_joint_table_.size(); }

void QNodeHUMANOID_ROBOT::clearUsingModule() {
  for (std::map<std::string, bool>::iterator map_it = using_mode_table_.begin();
       map_it != using_mode_table_.end(); ++map_it)
    map_it->second = false;
}

bool QNodeHUMANOID_ROBOT::isUsingModule(std::string module_name) {
...
void QNodeHUMANOID_ROBOT::moveInitPose() {
...
void QNodeHUMANOID_ROBOT::setControlMode(const std::string &mode) {
...
void QNodeHUMANOID_ROBOT::getJointControlMode() {
  ...
  if (get_module_control_client_.call(get_joint)) {
    ...
    for (int ix = 0; ix < get_joint.response.joint_name.size(); ix++) {
      ...
      std::map<std::string, int>::iterator service_it =
          service_map.find(joint_name);
      ...
      std::map<std::string, bool>::iterator module_it =
          using_mode_table_.find(module_name);
...
void QNodeHUMANOID_ROBOT::refreshCurrentJointControlCallback(
  ...
  for (int ix = 0; ix < getJointSize(); ix++) {
    ...
    std::map<std::string, int>::iterator module_it =
        joint_module_map.find(joint_name);
...
void QNodeHUMANOID_ROBOT::log(const LogLevel &level, const std::string &msg,
  ...
  case (Debug): {
    ...
    logging_model_msg << "[DEBUG] [" << min_str.str() << ":" << sec_str.str()
                      << "]: " << sender_ss.str() << msg;
    break;
  }
  case (Info): {
    ROS_INFO_STREAM(msg);
    logging_model_msg << "[INFO] [" << min_str.str() << ":" << sec_str.str()
                      << "]: " << sender_ss.str() << msg;
    break;
  }
  case (Warn): {
    ROS_WARN_STREAM(msg);
    logging_model_msg << "[WARN] [" << min_str.str() << ":" << sec_str.str()
                      << "]: " << sender_ss.str() << msg;
    break;
  }
  case (Error): {
    ROS_ERROR_STREAM(msg);
    logging_model_msg << "<ERROR> [" << min_str.str() << ":" << sec_str.str()
                      << "]: " << sender_ss.str() << msg;
    break;
  }
  case (Fatal): {
    ROS_FATAL_STREAM(msg);
    logging_model_msg << "[FATAL] [" << min_str.str() << ":" << sec_str.str()
                      << "]: " << sender_ss.str() << msg;
    break;
  }
  }
  QVariant new_row(QString(logging_model_msg.str().c_str()));
  logging_model_.setData(logging_model_.index(logging_model_.rowCount() - 1),
                         new_row);
  Q_EMIT loggingUpdated(); // used to readjust the scrollbar
}

void QNodeHUMANOID_ROBOT::clearLog() {
