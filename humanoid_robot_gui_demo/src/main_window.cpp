...
#include "../include/humanoid_robot_gui_demo/main_window.hpp"
#include <QMessageBox>
#include <QShortcut>
#include <QtGui>
#include <iostream>
...
MainWindow::MainWindow(int argc, char **argv, QWidget *parent)
  ...
  QObject::connect(
      ui_.actionAbout_Qt, SIGNAL(triggered(bool)), qApp,
   ...
   ** Logging
   **********************/
  ui_.view_logging->setModel(qnode_humanoid_robot_.loggingModel());
  QObject::connect(&qnode_humanoid_robot_, SIGNAL(loggingUpdated()), this,
                   SLOT(updateLoggingView()));

  /*********************
   ** Auto Start
   **********************/
  qnode_humanoid_robot_.init();
  initModeUnit();
  setUserShortcut();
  updateModuleUI();

  // Set Preview widget
  bool result = ui_.widget_preview_walking->init(&qnode_humanoid_robot_);
  if (result == false)
    exit(0);
}
...
void MainWindow::on_head_center_button_clicked(bool check) {
  qnode_humanoid_robot_.log(QNodeHUMANOID_ROBOT::Info, "Go Head init position");
  setHeadAngle(0, 0);
}

void MainWindow::on_button_demo_start_clicked(bool check) {
  qnode_humanoid_robot_.setModuleToDemo();

  usleep(10 * 1000);

  qnode_humanoid_robot_.setDemoCommand("start");
}

void MainWindow::on_button_demo_stop_clicked(bool check) {
  qnode_humanoid_robot_.setDemoCommand("stop");
}

void MainWindow::on_button_r_kick_clicked(bool check) {
  qnode_humanoid_robot_.setActionModuleBody();

  usleep(10 * 1000);

  qnode_humanoid_robot_.playMotion(RightKick);
}
...
void MainWindow::on_button_l_kick_clicked(bool check) {
  ...
  qnode_humanoid_robot_.playMotion(LeftKick);
}
...
void MainWindow::on_button_getup_front_clicked(bool check) {
  ...
  qnode_humanoid_robot_.playMotion(GetUpFront);
}
...
void MainWindow::on_button_getup_back_clicked(bool check) {
  ...
  qnode_humanoid_robot_.playMotion(GetUpBack);
}
 ...
 */
void MainWindow::updateLoggingView() { ui_.view_logging->scrollToBottom(); }
...
void MainWindow::setUserShortcut() {
  ...
  connect(_sig_map, SIGNAL(mapped(int)), ui_.tabWidget_control,
          SLOT(setCurrentIndex(int)));

  QShortcut *walking_shortcut =
      new QShortcut(QKeySequence(Qt::Key_Space), this);
  connect(walking_shortcut, SIGNAL(activated()), this,
          SLOT(walkingCommandShortcut()));
}
...
void MainWindow::setMode(bool check) {
  ...
  for (int ix = 0; ix < _combo_children.length(); ix++) {
    ...
    qnode_humanoid_robot_.log(QNodeHUMANOID_ROBOT::Info, _stream.str());
  }
  ...
  qnode_humanoid_robot_.log(QNodeHUMANOID_ROBOT::Info, "set mode");

  qnode_humanoid_robot_.setJointControlMode(_control_msg);
}
...
void MainWindow::updateModuleUI() {
  ...
  for (int index = 0; index < qnode_humanoid_robot_.getModeSize(); index++) {
    std::string _mode = qnode_humanoid_robot_.getModeName(index);
    if (_mode == "")
      continue;

    std::map<std::string, QList<QWidget *>>::iterator _module_iter =
        module_ui_table_.find(_mode);
    if (_module_iter == module_ui_table_.end())
      continue;

    bool _is_enable = qnode_humanoid_robot_.isUsingModule(_mode);
  ...
  if (qnode_humanoid_robot_.isUsingModule("walking_module"))
    qnode_humanoid_robot_.refreshWalkingParam();
}
...
void MainWindow::setHeadAngle(double pan, double tilt) {
  qnode_humanoid_robot_.setHeadJoint(pan * M_PI / 180, tilt * M_PI / 180);
}

// walking
void MainWindow::updateWalkingParams(
  ...
  ui_.dSpinBox_init_offset_roll->setValue(params.init_roll_offset *
                                          RADIAN2DEGREE);
  ui_.dSpinBox_init_offset_pitch->setValue(params.init_pitch_offset *
                                           RADIAN2DEGREE);
  ui_.dSpinBox_init_offset_yaw->setValue(params.init_yaw_offset *
                                         RADIAN2DEGREE);
  ui_.dSpinBox_hip_pitch_offset->setValue(params.hip_pitch_offset *
                                          RADIAN2DEGREE);
  // time
...
void MainWindow::applyWalkingParams() {
  humanoid_robot_walking_module_msgs::WalkingParam walking_param;
  ...
  walking_param.init_roll_offset =
      ui_.dSpinBox_init_offset_roll->value() * DEGREE2RADIAN;
  walking_param.init_pitch_offset =
      ui_.dSpinBox_init_offset_pitch->value() * DEGREE2RADIAN;
  walking_param.init_yaw_offset =
      ui_.dSpinBox_init_offset_yaw->value() * DEGREE2RADIAN;
  walking_param.hip_pitch_offset =
      ui_.dSpinBox_hip_pitch_offset->value() * DEGREE2RADIAN;
  // time
  walking_param.period_time =
      ui_.dSpinBox_period_time->value() * 0.001; // ms -> s
  ...
  walking_param.angle_move_amplitude =
      ui_.dSpinBox_a_move_amplitude->value() * DEGREE2RADIAN;
  ...
  walking_param.balance_ankle_pitch_gain =
      ui_.dSpinBox_ankle_pitch_gain->value();
  ...
  walking_param.pelvis_offset =
      ui_.dSpinBox_pelvis_offset->value() * DEGREE2RADIAN;
  walking_param.arm_swing_gain = ui_.dSpinBox_arm_swing_gain->value();

  qnode_humanoid_robot_.applyWalkingParam(walking_param);
}
...
void MainWindow::initModeUnit() {
  int number_joint = qnode_humanoid_robot_.getJointSize();
  ...
  // yaml preset
  for (std::map<int, std::string>::iterator module_it =
           qnode_humanoid_robot_.module_table_.begin();
       module_it != qnode_humanoid_robot_.module_table_.end(); ++module_it) {
    ...
    QObject::connect(preset_button, SIGNAL(clicked()), signalMapper,
                     SLOT(map()));
  }

  QObject::connect(signalMapper, SIGNAL(mapped(QString)), this,
  ...
  for (int ix = 0; ix < number_joint; ix++) {
    ...
    for (int index = 0; index < qnode_humanoid_robot_.getModeSize(); index++) {
      std::string module_name = qnode_humanoid_robot_.getModeName(index);
      if (module_name != "")
  ...
  for (int index = 0; index < qnode_humanoid_robot_.getModeSize(); index++) {
    ...
    QList<QWidget *> widget_list =
        ui_.centralwidget->findChildren<QWidget *>(reg_exp);
    module_ui_table_[module_name] = widget_list;

    if (debug_)
      std::cout << "Module widget : " << module_name << " ["
                << widget_list.size() << "]" << std::endl;
  }

  // make motion tab
  if (qnode_humanoid_robot_.getModeIndex("action_module") != -1)
    initMotionUnit();
}
...
void MainWindow::initMotionUnit() {
  ...
  // yaml preset
  int index = 0;
  for (std::map<int, std::string>::iterator motion_it =
           qnode_humanoid_robot_.motion_table_.begin();
       motion_it != qnode_humanoid_robot_.motion_table_.end(); ++motion_it) {
    ...
    // hotkey
    std::map<int, int>::iterator shortcut_it =
        qnode_humanoid_robot_.motion_shortcut_table_.find(motion_index);
    if (shortcut_it != qnode_humanoid_robot_.motion_shortcut_table_.end())
      motion_button->setShortcut(QKeySequence(shortcut_it->second));

    signal_mapper->setMapping(motion_button, motion_index);
    QObject::connect(motion_button, SIGNAL(clicked()), signal_mapper,
                     SLOT(map()));
  ...
  QObject::connect(signal_mapper, SIGNAL(mapped(int)), &qnode_humanoid_robot_,
                   SLOT(playMotion(int)));
...
void MainWindow::setMode(QString mode_name) {
  qnode_humanoid_robot_.setControlMode(mode_name.toStdString());
}

void MainWindow::readSettings() {
  QSettings settings("Qt-Ros Package", "humanoid_robot_gui_demo");
