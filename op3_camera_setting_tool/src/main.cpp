...
int main(int argc, char **argv) {
  ...
  ros::Subscriber camera_param_sub =
      nh.subscribe("/op3_camera/set_param", 1, &setCameraParameterCallback);
  ros::Subscriber camera_params_sub =
      nh.subscribe("/op3_camera/set_params", 1, &setCameraParametersCallback);

  // web setting
  ros::NodeHandle nh_param("~");
  g_has_path = nh_param.getParam("yaml_path", g_param_path);

  g_param_pub = nh.advertise<op3_camera_setting_tool::CameraParams>(
      "/op3_camera/camera_params", 1);
  g_param_command_sub =
      nh.subscribe("/op3_camera/param_command", 1, &paramCommandCallback);
  g_set_param_client =
      nh.advertiseService("/op3_camera/set_camera_params", &setParamCallback);
  g_get_param_client =
      nh.advertiseService("/op3_camera/get_camera_params", &getParamCallback);
  g_default_setting_path = ros::package::getPath(ROS_PACKAGE_NAME) +
                           "/config/camera_parameter_default.yaml";

  boost::recursive_mutex config_mutex;

  // dynamic_reconfigure::Server<op3_camera_setting_tool::CameraParamsConfig>
  // param_server;
  dynamic_reconfigure::Server<
      op3_camera_setting_tool::CameraParamsConfig>::CallbackType callback_fnc;
  g_param_server.reset(
      new dynamic_reconfigure::Server<
          op3_camera_setting_tool::CameraParamsConfig>(config_mutex));
...
void dynParamCallback(op3_camera_setting_tool::CameraParamsConfig &config,
                      uint32_t level) {
  g_dyn_config = config;
...
void setCameraParameterCallback(
    const op3_camera_setting_tool::V4lParameter::ConstPtr &msg) {
...
void setCameraParametersCallback(
    const op3_camera_setting_tool::V4lParameters::ConstPtr &msg) {
...
bool setParamCallback(op3_camera_setting_tool::SetParameters::Request &req,
                      op3_camera_setting_tool::SetParameters::Response &res) {
...
bool getParamCallback(op3_camera_setting_tool::GetParameters::Request &req,
                      op3_camera_setting_tool::GetParameters::Response &res) {
